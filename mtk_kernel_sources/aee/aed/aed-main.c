...
static void ke_gen_userbacktrace_msg(void)
{
	...
	memcpy( (data+ sizeof(pid_t)+sizeof(int)), (char *)(aed_dev.kerec.lastlog->userthread_stack.Userthread_Stack),aed_dev.kerec.lastlog->userthread_stack.StackLength);
...
static void ke_gen_usermaps_msg(void)
{
	...
	memcpy( (data+ sizeof(pid_t)+sizeof(int)), (char *)(aed_dev.kerec.lastlog->userthread_maps.Userthread_maps),aed_dev.kerec.lastlog->userthread_maps.Userthread_mapsLength);
	...
	case AEEIOCTL_DAL_CLEAN:
		{
			...
			dal_setcolor.foreground = 0x00ff00;	
			dal_setcolor.background = 0xff0000;	
...
void aee_kernel_dal_api(const char *file, const int line, const char *msg)
{
	...
	if (msg != NULL) {
		...
		if (((aee_mode == AEE_MODE_MTK_ENG) && (force_red_screen == AEE_FORCE_NOT_SET))
			...
			dal_setcolor.foreground = 0xff00ff;	
			dal_setcolor.background = 0x00ff00;	
			DAL_SetColor(dal_setcolor.foreground, dal_setcolor.background);
			dal_setcolor.screencolor = 0xff0000;	
			DAL_SetScreenColor(dal_setcolor.screencolor);
