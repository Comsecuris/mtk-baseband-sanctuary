...
#define MD_STATUS_POLLING_TIMEOUT 15
inline void ccci_port_debug_scan_busy_tasks()
{
...
static inline void ccci_port_debug_scan_disk_sleep_tasks()
{
...
void md_status_poller_func(unsigned long data)
{
    ...
    mod_timer(&md->md_status_timeout, jiffies + MD_STATUS_POLLING_TIMEOUT*HZ);
...
void md_status_timeout_func(unsigned long data)
{
	...
	ccci_port_debug_scan_busy_tasks();
...
static void ccci_ee_info_dump(struct ccci_modem *md)
{
	...
	if ( (get_kernel_flag() & KERNEL_FLAG_RIL_DBG_MEMCPY) ) {
		...
		CCCI_INF_MSG(md->index, KERN, "Dump MD EX log disabled\n");
	}
