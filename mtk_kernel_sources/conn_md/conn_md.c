...
int conn_md_send_msg(ipc_ilm_t *ilm)
{
	...
	if (NULL != p_new_msg) {
		...
		CONN_MD_DBG_FUNC("ilm:0x%08x, msg_len:%d, p_new_msg:0x%08x, p_local_para:0x%08x, msg_len:%d\n",
			ilm, ilm->local_para_ptr->msg_len, p_new_msg,
		...
		CONN_MD_DBG_FUNC("ilm:0x%08x, msg_len:%d, kmalloc for new message structure failed\n", ilm, ilm->local_para_ptr->msg_len);
	}
...
static int conn_md_thread(void *p_data)
{
	...
	while (1) {
		...
		list_for_each(pos, &p_act_queue->list) {
			...
			CONN_MD_DBG_FUNC("sending message: source user id (0x%08x), target user id (0x%08x), p_cur_ilm:0x%08x, local_para_ptr:0x%08x, msg_len:%d\n",
					 p_cur_ilm->src_mod_id, p_cur_ilm->dest_mod_id,
