...
static struct mtk_uart_setting mtk_uart_default_settings[] = {
	{
	 ...
	 .tx_trig = UART_FCR_TXFIFO_1B_TRI, .rx_trig = UART_FCR_RXFIFO_12B_TRI,
        ...
        .set_bit = PDN_FOR_UART0,  .clr_bit = PDN_FOR_UART0,  .pll_id = PDN_FOR_UART0,
...
void set_uart_default_settings(int idx)
{
    ...
    u32 phys_base;
#if defined(CONFIG_SERIAL_CIR)
    struct device_node *node_cir = NULL;
    int idx_cir = 0;

    node_cir = of_find_compatible_node(NULL, NULL, "CIR");
    if(node_cir)
	of_property_read_u32(node_cir, "cell-index", &idx_cir);
    else
	idx_cir = -1;

    if (idx == idx_cir) {
	printk("[CIR] [UART%d] %s\n", idx_cir, __func__);
	mtk_uart_default_settings[idx].tx_mode = UART_NON_DMA;
	mtk_uart_default_settings[idx].rx_mode = UART_NON_DMA;
	mtk_uart_default_settings[idx].dma_mode = UART_DMA_MODE_0;
	mtk_uart_default_settings[idx].sysrq = FALSE;
	mtk_uart_default_settings[idx].hw_flow = FALSE;
	mtk_uart_default_settings[idx].vff = FALSE;
    }
#endif
...
inline static void dump_reg(struct mtk_uart *uart, const char *caller)
{
	...
	MSG(CFG,
	    "%s: RATEFIX(%02X); HSPEED(%02X); CNT(%02X); PNT(%02X); DLH(%02X), DLL(%02X), IER(%02X)\n",
...
void dump_uart_reg(void)
{
	...
	for (i = 0; i < UART_NR; i++) {
		...
		if (uart->poweron_count > 0) {
			...
			pr_notice("[%d] LSR=0x%x   ESCAPE_EN=0x%x\n", uart->nport, lsr, escape_en);
		} else
			pr_notice("[%d] clock is off\n", uart->nport);
	}
...
static void mtk_uart_dma_vfifo_tx_tasklet_byte(unsigned long arg)
{
	...
	while (len--) {
		while (UART_READ32(VFF_FLUSH(vfifo->base))) {
			...
			if ((b.tv_sec - a.tv_sec) > 1
				...
				pr_notice("[%d] Polling flush timeout\n", port->line);
				return;
			}
		}
	...
	if (UART_DEBUG_EVT(DBG_EVT_DMA) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		MSG(DMA, "TX[%4d]: %4d/%4d [%05X-%05X] (%02X %02X .. %02X %02X)\n",
		    ...
		    UART_READ32(VFF_RPT(vfifo->base)), str[0], str[1], str[2], str[3]);
	} else {
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		int i;
		pr_notice("[%d_TX] %4d bytes:", uart->nport, vfifo->cur->idx);
		for (i = 0; i < vfifo->cur->idx; i++) {
			if (i % 16 == 0)
				pr_notice("\n");
			pr_notice("%.2x ", (unsigned char)vfifo->cur->dat[i]);
		}
		pr_notice("\n");
	}
#endif
...
static void mtk_uart_dma_vfifo_rx_tasklet_str(unsigned long arg)
{
	...
	count = left =
	    ((rxreg ^ txreg) & 0x00010000) ? (txptr + vfifo->size - rxptr) : (txptr - rxptr);
	...
	if (UART_DEBUG_EVT(DBG_EVT_DMA) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		...
		MSG(DMA, "RX[%4d]: [%5X..%5X] [%5X..%5X] [%d] [%4X.%4X]\n",
		    left, rxreg, txreg, UART_READ32(VFF_RPT(base)), UART_READ32(VFF_WPT(base)),
		    UART_READ32(VFF_FLUSH(base)), UART_READ32(VFF_VALID_SIZE(base)),
	...
	if (UART_DEBUG_EVT(DBG_EVT_DAT) && UART_DEBUG_EVT(DBG_EVT_BUF)) {
		int i;
		pr_notice("[%d_RX] %4d bytes:", uart->nport, vfifo->cur->idx);
...
static void mtk_uart_cal_baud(struct mtk_uart *uart, int baudrate, int highspeed)
{
	...
	MSG(CFG, "BaudRate = %d, SysClk = %d, Divisor = %d, %04X/%04X\n", baudrate, uartclk,
	    divisor, UART_READ32(UART_IER), UART_READ32(UART_LCR));
...
void mtk_uart_get_modem_status(struct mtk_uart *uart)
{
	...
	MSG(INFO, "MSR: DCD(%d), RI(%d), DSR(%d), CTS(%d)\n",
	    ...
	    status & UART_MSR_DSR ? 1 : 0, status & UART_MSR_CTS ? 1 : 0);
...
unsigned int mtk_uart_get_mctrl(struct uart_port *port)
{
	...
	MSG(INFO, "MSR: OUT1(%d), OUT2(%d), LOOP(%d)\n",
	    ...
	    status & UART_MCR_OUT2 ? 1 : 0, status & UART_MCR_LOOP ? 1 : 0);
...
void dump_uart_history(void)
{
	...
	for (i = 0; i < RECORD_NUMBER; i++) {
		...
		for (j = 0; j < uart_history_cnt[curr]; j++) {
			if ((j % 0xF) == 0)
				pr_notice("\n");
			pr_notice("%02x ", uart_history[curr][j]);
		}
		msleep(20);
		curr++;
		if (curr >= RECORD_NUMBER)
	...
	for (i = 0; i < RECORD_NUMBER; i++) {
		...
		for (j = 0; j < uart_rx_history_cnt[rx_curr]; j++) {
			if ((j % 0xF) == 0)
				pr_notice("\n");
			pr_notice("%02x ", uart_rx_history[rx_curr][j]);
		}
		msleep(20);
		rx_curr++;
		if (rx_curr >= RECORD_NUMBER)
...
#else
		pr_err("GPIO_UART_UTXD0_PIN is not properly set\n");
#endif
		break;
	case 1:
...
#else
		pr_err("GPIO_UART_UTXD1_PIN is not properly set\n");
#endif
		break;
	case 2:
...
#else
		pr_err("GPIO_UART_UTXD2_PIN is not properly set\n");
#endif
		break;
	default:
		break;
...
#else
		pr_err("GPIO_UART_UTXD0_PIN is not properly set p2\n");
#endif
		break;
	case 1:
#ifdef GPIO_UART_UTXD1_PIN
...
#else
		pr_err("GPIO_UART_UTXD1_PIN is not properly set p2\n");
#endif
		break;
	case 2:
#ifdef GPIO_UART_UTXD2_PIN
...
#else
		pr_err("GPIO_UART_UTXD2_PIN is not properly set p2\n");
#endif
		break;
	default:
		break;
...
#else
		pr_err("GPIO_UART_URXD0_PIN is not properly set\n");
#endif
		break;
	case 1:
...
#else
		pr_err("GPIO_UART_URXD1_PIN is not properly set\n");
#endif
		break;
	case 2:
...
#else
		pr_err("GPIO_UART_URXD2_PIN is not properly set\n");
#endif
		break;
	case 3:
...
#else
		pr_err("GPIO_UART_URXD3_PIN is not properly set\n");
#endif
		break;
	default:
		break;
...
#else
		pr_err("GPIO_UART_URXD0_PIN is not properly set p2\n");
#endif
		break;
	case 1:
#ifdef GPIO_UART_URXD1_PIN
...
#else
		pr_err("GPIO_UART_URXD1_PIN is not properly set p2\n");
#endif
		break;
	case 2:
#ifdef GPIO_UART_URXD2_PIN
...
#else
		pr_err("GPIO_UART_URXD2_PIN is not properly set p2\n");
#endif
		break;
	case 3:
#ifdef GPIO_UART_URXD3_PIN
...
#else
		pr_err("GPIO_UART_URXD3_PIN is not properly set p2\n");
#endif
		break;
	default:
		break;
