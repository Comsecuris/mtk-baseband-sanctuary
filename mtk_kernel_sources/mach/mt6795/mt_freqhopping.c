...
static const struct freqhopping_ssc ssc_armca15pll_setting[]= {
	{0,0,0,0,0,0},
	{0,0xFF,0xFF,0xFF,0xFF,0xFF},
	{ARMCA15PLL_DEF_FREQ, 0, 9, 0, 8, 0x10800}, 
	{0,0,0,0,0,0}
};
...
enum {
	MT_FH_DEBUG_DDS = BIT(0),
	MT_FH_DEBUG_DVFS = BIT(1),
};

static int mt_freqhopping_debug_mask = 0;
module_param_named(
	debug_mask, mt_freqhopping_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP
);
    ...
    unsigned int wait_count)
{
	...
	if (mt_freqhopping_debug_mask & MT_FH_DEBUG_DDS)
...
static int mt_fh_hal_dvfs(enum FH_PLL_ID pll_id, unsigned int dds_value)
{
	...
	if (mt_freqhopping_debug_mask & MT_FH_DEBUG_DVFS)
...
static int mt_fh_hal_dfs_mpll(unsigned int target_dds)
{
	...
	if(g_fh_pll[pll_id].fh_status == FH_FH_ENABLE_SSC)
    {
        unsigned int setting_index = MPLL_SSC_TBL_DEFAUL; 
        const struct freqhopping_ssc* p_setting;
#if defined(CONFIG_ARCH_MT6795)	       
        if(target_dds == MPLL_DDS_1333)
        {
            setting_index = MPLL_SSC_TBL_1333; 
        }
#endif       
