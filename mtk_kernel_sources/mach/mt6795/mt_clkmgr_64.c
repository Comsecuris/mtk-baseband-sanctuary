...
#endif
    clk_writel(clk_rgu_base+0x54, 0x80000000);
...
static struct clkmux muxs[NR_MUXS] = {
    {
    	...
    	.name = __stringify(MUX_AXI_MFG_IN_AS),
        .sel_mask = 0x00030000,
        .pdn_mask = 0x00800000,
        .offset = 0,
        .nr_inputs = 3,
    	...
    	.name = __stringify(MEM_MFG_IN_AS),
        .sel_mask = 0x00000300,
        .pdn_mask = 0x00008000,
        .offset = 0,
        .nr_inputs = 3,
...
return 0;
    ...
    clk_ver("[%s]: id=%d, clksrc=%d, name=%s\n", __func__, id, clksrc, name);
...
{
#ifdef CLK_LOG
    clk_ver("[%s]: clk->grp->name=%s, clk->mask=0x%x\n", __func__, clk->grp->name, clk->mask);
#endif    
...
{
    clk_ver("[%s]: Dump Top MUX register, CLK_CFG_0=0x%x, CLK_CFG_1=0x%x, CLK_CFG_2=0x%x, \
...
void clkmux_sel_for_vcorefs(bool mode)
{
    ...
    {   
        ...
        mux_disable_internal(mux, "clkmgr");
#else
        mt_fh_hal_dfs_vcodecpll(0x130000); 
#endif
        pll_disable_internal(pll, "clkmgr");
    }
    else
    {
        ...
        mt_fh_hal_dfs_vcodecpll(0xC7627); 
#endif
        pll_disable_internal(pll, "clkmgr");
        ...
        mux = id_to_mux(MT_MUX_MM);
        mux_enable_internal(mux, "clkmgr");
        clkmux_sel_locked(mux, 3); 
        mux_disable_internal(mux, "clkmgr");
#else
        mt_fh_hal_dfs_vencpll(0xC313B); 
#endif
        pll_disable_internal(pll, "clkmgr");  
        
        
        mux = id_to_mux(MT_MUX_AXI);
        clkmux_sel_locked(mux, 2); 
    }
