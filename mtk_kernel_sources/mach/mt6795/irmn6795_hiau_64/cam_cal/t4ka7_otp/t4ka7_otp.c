...
#define PFX "t4ka7_otp"

#define CAM_CALINF(fmt, arg...)    xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " fmt, __FUNCTION__, ##arg)
#define CAM_CALDB(fmt, arg...)    xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " fmt, __FUNCTION__, ##arg)
...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, T4KA7_OTP_DEVICE_ID>>1)};
...
static long t4ka7otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] t4ka7otp_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
#endif
char t4ka7_otp_data[23] = {0x01,0xff,0x00,0x0b,0x01,0xff,0xff,0x01,0x0c,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xff,0xff,0xff,0xff,00,00};
static int selective_read_region(u32 offset, BYTE* data,u16 i2c_id,u32 size)
{
    memcpy((void *)data,(void *)&t4ka7_otp_data[offset],size);
...
#endif
{
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            ...
            i4RetValue = selective_read_region(ptempbuf->u4Offset, pu1Params, T4KA7_OTP_DEVICE_ID,ptempbuf->u4Length);
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = t4ka7otp_Ioctl_Compat,
#endif
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALERR("[S24CAM_CAL] Allocate device no failed\n");
    ...
    {
        CAM_CALERR("[S24CAM_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALERR("[S24CAM_CAL] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALERR("[S24CAM_CAL] Attatch file operation failed\n");
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = T4KA7_OTP_DEVICE_ID>>1;
...
static int __init CAM_CAL_i2C_init(void)
{
   ...
   //Register char driver
    ...
    if(platform_driver_register(&g_stCAM_CAL_Driver)){
        CAM_CALERR("failed to register S24CAM_CAL driver\n");
        return -ENODEV;
    }

    if (platform_device_register(&g_stCAM_CAL_Device))
    {
        CAM_CALERR("failed to register S24CAM_CAL driver, 2nd time\n");
        return -ENODEV;
    }
