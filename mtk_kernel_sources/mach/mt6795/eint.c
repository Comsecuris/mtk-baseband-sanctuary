...
typedef struct
{
    ...
    int timer_inited[EINT_MAX_CHANNEL]; 
} eint_func;
...
static void mt_eint_set_timer_event(unsigned int eint_num)
{
    ...
    if(!EINT_FUNC.timer_inited[eint_num]){
        init_timer(eint_timer);
        add_timer_on(eint_timer, cpu);
        EINT_FUNC.timer_inited[eint_num]++;
    }else
        mod_timer(eint_timer, jiffies + msecs_to_jiffies(EINT_FUNC.deb_time[eint_num]));
}
...
void test_init()
{
        ...
        {
            if((i== 180)||(i==179)||(i==185)||(i==114)||(i==176)||(i==177)||(i==178))    
            {
                        gEint_test_skip[i] = 1;
            }
            if(i>=109 && i<=191)
...
void print_EINT_DBG_info(int eint_num){
   ...
   printk ("EINT_MASK 0x%x\n",(readl(IOMEM(EINT_MASK_BASE+offset*4))& (0x1<<shifft))>>shifft);              
   printk ("EINT_SENS 0x%x\n",(readl(IOMEM(EINT_SENS_BASE+offset*4))& (0x1<<shifft))>>shifft);                 
   printk ("EINT_SOFT_BASE 0x%x\n",(readl(IOMEM(EINT_SOFT_BASE+offset*4))& (0x1<<shifft))>>shifft);          
   printk ("EINT_POL_BASE 0x%x\n",(readl(IOMEM(EINT_POL_BASE+offset*4))& (0x1<<shifft))>>shifft);             
