...
#define ISP_WR32(addr, data)    mt65xx_reg_sync_writel(data, addr)    
...
static volatile spinlock_t SpinLockEDBufQueList;
...
typedef struct
{
    ...
    MINT8               WaitQIdx[ISP_IRQ_TYPE_AMOUNT*32]; 
...
LOG_DBG("+LARB in DEQUE,BWL(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X),220(0x%08X)/(0x%08X),0x14(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)/(0x%08X)", \
    ...
    ISP_RD32(SMI_LARB4+0x10));
		...
		switch (rt_dma) {
			...
			memset(IspInfo.IrqInfo.Eismeta[ISP_IRQ_TYPE_INT_P1_ST],0,sizeof(ISP_EIS_META_STRUCT)*EISMETA_RINGSIZE);
                                gEismetaRIdx = 0;
                                gEismetaWIdx = 0;
                                gEismetaInSOF = 0;
			...
			memset(IspInfo.IrqInfo.Eismeta[ISP_IRQ_TYPE_INT_P1_ST_D],0,sizeof(ISP_EIS_META_STRUCT)*EISMETA_RINGSIZE);
                                gEismetaRIdx_D = 0;
                                gEismetaWIdx_D = 0;
                                gEismetaInSOF_D = 0;
